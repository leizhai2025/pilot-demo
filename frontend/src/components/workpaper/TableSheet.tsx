...
export function TableSheet({ sheet, rows, onChangeRows }: Props) {
          ...
          <span className="ml-auto flex items-center gap-3 not-italic">
            {isFilled && (
              <span className="inline-flex items-center gap-1 text-[11px] text-brand-700">
                <Sparkles size={11} /> AI 已填 {rows.length} 行
              </span>
            )}
            {anomalies > 0 ? (
...
function Cell({
  col, sheetCode, row, onChange,
}: { col: SheetColumn; sheetCode: string; row: Record<string, any>; onChange: (v: any) => void }) {
  ...
  if (col.type === 'money' || col.type === 'number') {
    return (
      <td className="px-1 py-0.5 border-r border-slate-100 text-right">
        <input
